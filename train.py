...
def get_cnn_model(num_classes, lr):
    l2_reg = 0.001
    opt = Adam(lr = lr)
...
def train(train_path, epochs, batch_size, learning_rate):
    ...
    np.save('classes.npy', lb_encod.classes_)
    ...
    cnn_model = get_cnn_model(num_classes, learning_rate) 
    filepath = "weights.hdf5"
    checkpoint = ModelCheckpoint(filepath=filepath, monitor='loss', verbose=1, save_best_only=True, mode='min')
    history = cnn_model.fit(x_train, y_train, batch_size = batch_size, epochs = epochs, verbose = 1,validation_split = 0.33)
